...
const getMargin = async (params) => {
    ...
    utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Retrieving Margin from Bitmex`)
...
const getOrders = async (params) => {
    ...
    utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Retrieving ${params.type} Orders from Bitmex`)
...
const setOrders = async (params) => {
    ...
    utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Setting a ${params.orderType} ${params.side} Order of ${params.symbol} for ${params.orderQty} @ $${params.price}`)
...
const cancelOrders = async (params) => {
    const bitMexClient = await getClient(params.keys)
    utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Cancellin Order with ID: ${params.orderId}`)
...
const setLeverage = async (params) => {
    ...
    utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Setting leverage of ${params.symbol} to ${params.leverage}x`)
...
const getPositions = async (params) => {
    ...
    utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Push ${params.symbol} position from ${params.botId} to Kafka`)
    await bitMexClient.monitorStream(`${params.symbol}`, "position", async (data) => {
        const positionObject = {
            bot_id: params.bot_id,
            data
        }
        await utils.kafkaProduce(topic, positionObject)
    });
}
...
const getHistory = async (params) => {
    ...
    utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Retrieving PriceHistory from ${params.startTime} to ${params.endTime}`)
...
const streamPrice = async (params) => {
    ...
    utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Initialising stream for ${params.timeFrame}${params.asset}`)
    await bitMexClient.monitorStream(params.asset, `tradeBin${params.timeFrame}`, async (data, e) => {
        ...
        utils.kafkaProduce(topic, streamingObject)
    });
}
...
const getClient = async (params) => {
        try {
            utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Creating a Bitmex client to retrieve data`)
